...
function removeConfirm(){
    ...
    if (row){
    	$.messager.confirm('信息提示', '确认要删除吗?', function(r){
            if (r){
            	$.get(contextPath+"/states_delete?stateId="+row.stateId,null,
					function(data,state){
...
function loadRemote(){
    ...
    if (row){
        $('#edit').dialog({
    		onLoad :function(){
    			//回显数据
    			$('#editForm').form('load', contextPath+'/states_update?stateId='+row.stateId);
    		}
    	}).dialog('open');
    }else{
	...
	$("#add-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/states_addSave",datas,
						function(data,state){
	   	            		var dt=eval("("+data+")");//把json字符串转换成对象
	            			if(dt.status==1){
...
$(function(){//保存修改
	$("#edit-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/states_updateSave",datas,
						function(data,state){
	   	            		var dt=eval("("+data+")");//把json字符串转换成对象
	            			if(dt.status==1){
								$.messager.alert('信息提示','保存成功');
...
$(function(){
    ...
    $('#dg').datagrid({
    	columns:[[
			{field:'stateId',title:'编号', width:80},
			{field:'stateName',title:'功能名称', width:160}
    	]]
    });
