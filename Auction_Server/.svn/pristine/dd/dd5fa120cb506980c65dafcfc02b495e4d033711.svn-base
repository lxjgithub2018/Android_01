...
public abstract class AbstractBaseDao<T, K extends Serializable> implements IBaseDao<T, K> {
	...
	@Resource //告诉spring，当扫描到这里时，注入hibernate会话工厂对象进来，初始此属性
	private SessionFactory sessionFactory;//hibernate的会话工厂
	...
	public Integer getSeq(String sql) throws Exception {
		BigDecimal seq= (BigDecimal)this.getSession().createNativeQuery(sql).getResultList().get(0);
		return seq.intValue();
	}
	...
	public List<T> findEntity(final String hql, final Integer start, final Integer limit) throws Exception {
		...
		query.setFirstResult(start).setMaxResults(limit);//设置查找起始位置与结束位置
		return query.getResultList();
	}
