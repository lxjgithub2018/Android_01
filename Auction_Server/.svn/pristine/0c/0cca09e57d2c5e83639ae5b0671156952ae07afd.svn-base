...
function removeConfirm(){
    ...
    if (row){
    	$.messager.confirm('信息提示', '确认要删除吗?', function(r){
            if (r){
            	$.post(contextPath+"/kinds_delete?kindId="+row.kindId,null,
					function(data,state){
...
function loadRemote(){
    ...
    if (row){
    	$('#edit').dialog({
    		onLoad :function(){
    			//回显数据
    			$('#editForm').form('load', contextPath+'/kinds_update?kindId='+row.kindId);
    		}
    	}).dialog('open');
	...
	$("#add-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/kinds_addSave",datas,
						function(data,state){
   	            			var dt=eval("("+data+")");//把json字符串转换成对象
   	            			if(dt.status==1){
...
$(function(){//保存修改
	$("#edit-save-button").click(
		function(){
			...
			$.messager.confirm('信息提示', '确认要保存吗?', function(r){
   	            if (r){
   	            	$.post(contextPath+"/kinds_updateSave",datas,
						function(data,state){
	   	            		var dt=eval("("+data+")");//把json字符串转换成对象
		        			if(dt.status==1){
								$.messager.alert('信息提示','保存成功');
...
$(function(){
    ...
    $('#dg').datagrid({
    	columns:[[
			{field:'kindId',title:'编号', width:80},
			{field:'kindName',title:'类别名称', width:160},
			{field:'kindDesc',title:'类别备注', width:160}
    	]]
    });
